(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["plain-utils"] = factory();
	else
		root["plain-utils"] = factory();
...
var util = __webpack_require__("3a40")
...
var engine = __webpack_require__("a7fe")

var storages = __webpack_require__("cbe4")
var plugins = [__webpack_require__("e280")]
...
module.exports = [
	...
	__webpack_require__("5eb3"),
	__webpack_require__("7e14"),
	__webpack_require__("3ac5"),
...
// CONCATENATED MODULE: ./node_modules/_fecha@3.0.3@fecha/src/fecha.js
/**
...
// CONCATENATED MODULE: ./src/scripts/keyboard.js
...
// CONCATENATED MODULE: ./src/scripts/json-parse.js



var json_parse_parseJson = function parseJson(json, caretToDebug) {
  var debug = typeof caretToDebug === 'number';
  ...
  var lex = function lex(_ref) {
    var state = _ref.state,
        head = _ref.head,
        char = _ref.char,
        insert = _ref.insert,
        remove = _ref.remove,
        removePreviousComma = _ref.removePreviousComma,
    ...
    var _ret = function () {
      ...
      for (i = 0, j = 0; i < mutableJson.length; i++, j++) {
        ...
        state = lex({
          ...
          head: stack[stack.length - 1],
          char: char,
          insert: function insert(charToInsert) {
            mutableJson = mutableJson.slice(0, i) + charToInsert + mutableJson.slice(i);
          },
          remove: function remove() {
            mutableJson = mutableJson.slice(0, i) + mutableJson.slice(i + 1);
            i--;
          },
          removePreviousComma: function removePreviousComma() {
            if (mutableJson[i - 1] === ',') {
              mutableJson = mutableJson.slice(0, i - 1) + mutableJson.slice(i);
              i--;
            }
          },
          replace: function replace(charToInsert) {
            mutableJson = mutableJson.slice(0, i) + charToInsert + mutableJson.slice(i + 1);
          },
      ...
      if (state === 'obj_complete') {
        try {
          var _obj = JSON.parse(mutableJson);
...
function decodeDate(date) {
  ...
  ret.timeString = "".concat(zeroize(ret.hour), ":").concat(zeroize(ret.minute), ":").concat(zeroize(ret.second));
  return ret;
}
/**
 * 使用px单位化
...
function decodeUrl(url) {
  ...
  if (urlParts.length === 2) {
    var paramStrings = urlParts[1].split("&");
    paramStrings.forEach(function (str) {
...
function copyToClipboard(text, success, error) {
  ...
  try {
    ...
    if (successful) !!success && success();else !!error && error("复制到剪切板失败！");
  } catch (err) {
    !!error && error(err);
  }
...
function chunk(array, size) {
  ...
  var index = 0;
  var resIndex = 0;
  var result = new Array(Math.ceil(length / size)); // #3
...
var $utils = {
  ...
  parseJson: json_parse_parseJson,
  //解析json
...
// CONCATENATED MODULE: ./src/scripts/dom.js
...
function hasClass(el, cls) {
  if (!!el.classList && !!el.classList.value) return utils.oneOf(cls, el.classList.value.split(' '));else return (el.className || '').split(' ').indexOf(cls) > -1;
}
/**
 * 添加class
 * @author 韦胜健
...
function addClass(el, addCLs) {
  if (!el || !addCLs) return;
  var addClasses = utils.typeOf(addCLs) === 'string' ? addCLs.split(' ') : addCLs;

  if (!!el.classList && !!el.classList.value) {
    addClasses.forEach(function (item) {
...
function removeClass(el, rmCls) {
  if (!el || !rmCls) return;
  var rmClasses = utils.typeOf(rmCls) === 'string' ? rmCls.split(' ') : rmCls;

  if (!!el.classList && !!el.classList.value) {
    rmClasses.forEach(function (item) {
      return el.classList.remove(item);
    });
  } else {
    var curClasses = (el.className || '').split(' ');

    for (var i = 0; i < rmClasses.length; i++) {
      var rmClass = rmClasses[i];
      var index = curClasses.indexOf(rmClass);
...
// CONCATENATED MODULE: ./src/scripts/storage.js




var store = __webpack_require__("9b2d");

var storage_StorageService =
/*#__PURE__*/
function () {
  function StorageService() {
    var storageKey = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'plain';

    _classCallCheck(this, StorageService);

    _defineProperty(this, "store", store);
...
/* harmony default export */ var storage = (storage_StorageService);
...
/* harmony default export */ var src = (PlainUtils);
...
//# sourceMappingURL=plain-utils.umd.js.map